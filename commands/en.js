...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (repliedMessage) {
          const data = await fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=en&dt=t&dj=1&q=${encodeURIComponent(repliedMessage)}`)
          .then(res=>res.json())
          ...
          const data = await fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=en&dt=t&dj=1&q=${encodeURIComponent(content)}`)
          .then(res=>res.json())
          const translated = data.sentences.map((sentence) => {
            return sentence.trans;
          });

          message.channel.send(`[${data.src}]->[en]\n${translated.join("")}`)
          .then(sentMessage => {
              message.react('⭕');
          });
        }

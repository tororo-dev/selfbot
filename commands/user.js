...
        if (!user) {
        ...
        const userInfo = `名前:%20${user.displayName}%0D%0A` +
            `タグ:%20${user.tag}%0D%0A` +
            `ID:%20${user.id}%0D%0A` +
            `作成日:%20${user.createdAt.toISOString().replace(/T/, '%20').replace(/\..+/, '')}`;
        const avatarURL = `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png?size=1024`;

        const embed = new WebEmbed()
        .setColor('GREEN')
        .setDescription(`タグ: ${user.tag}\nID: ${user.id}\n作成日: ${user.createdAt.toISOString()}`)
        .setProvider({ name: `@${message.author.username}`, url: `https://discord.com/users/${message.author.id}` })
